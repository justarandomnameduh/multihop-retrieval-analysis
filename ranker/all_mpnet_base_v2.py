...
def gen_stage_0(corpus, queries, rank_model_name, rerank, rerank_model_name, output_name):
    ...
    # Disable OpenAI LLM
    llm = None  # No LLM used

    # Remove any previous save file
    print('Removing save file if it exists...')
    rm_file(save_file)

    # Create embedding model with HuggingFace
    ...
    # Initialize the ingestion pipeline and process the corpus data
    ...
    for data in tqdm(query_data):
        ...
        # Retrieve the top nodes (documents)
        if rerank:
        ...
        # Prepare the retrieval results for saving
        retrieval_list = []
...
if __name__ == '__main__':
  if STAGING:
    ...
    output_name = "output/all_mpnet_base_v2.json"
  else:
